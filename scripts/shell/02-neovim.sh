...
# Back up only if the existing config was NOT generated by this script.
# If our marker is present, we overwrite silently — re-running should always
# produce a clean, current state (this is how the treesitter tag pin gets applied
# on subsequent runs without manual file editing).
if [[ -f "${NVIM_CONFIG_DIR}/init.lua" ]] && ! grep -q "fedora-workstation-setup" "${NVIM_CONFIG_DIR}/init.lua" 2>/dev/null; then
  ...
  log_warn "Foreign Neovim config found — backing up to ${NVIM_BACKUP}"
  if ! "${DRY_RUN}"; then
